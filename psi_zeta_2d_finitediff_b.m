...
Ah= 1.e-5;  %diffusion
bta=0.005; %strength of the beta effect 2.omega.cos(phi)
Amp=1;
...
while( ts<Nts)
    ...
    if mod(ts,40)==0
        ...
        if ts==200
            imwrite(imind,cm,'assignment2_3_eddysimulate_beta.gif','gif', 'Loopcount',inf);
        else
            imwrite(imind,cm,'assignment2_3_eddysimulate_beta.gif','gif','WriteMode','append');
        end
    end
end
